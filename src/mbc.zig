    ...
    pub fn read_byte(self: MBC1, address: u16) u8 {
        switch (address) {
            ...
            0xA000...0xBFFF => {
                ...
                const offset = @as(u16, self.ram_bank) * 8192;
                const index = @as(usize, offset + address) - 0xA000;
                std.debug.print("\naddress: {X}, bank: {d}, offset: {d}, index: {d}", .{ address, self.ram_bank, offset, index });
        ...
        switch (address) {
            ...
            0x4000...0x5FFF => {
                if (self.default_mode) {
                    self.ram_bank = @as(u2, @truncate(value)) & 0b11;
                } else {
                    self.rom_bank = self.rom_bank & 0b00011111 | (value & 0b11) << 5;
                }
            },
            0x6000...0x7FFF => self.default_mode = value == 1,
            else => {},
        }
    }
